...
public class Main {
	...
	int getStickInputCount(Scanner scanner) { 
		int count = scanner.nextInt();
		return count;
	}
	
	int[] getStickInput(int count, Scanner scanner) { 
		if (count != 0) {
	...
	public static void main(String[] args) {
		...
		int count;
		int[] sticks;
		while ((count = solution.getStickInputCount(scanner)) != 0) {
			sticks = solution.getStickInput(count, scanner);
			int maxStick = 0;
			int totalLength = 0;
			for (int i : sticks) {
				if (maxStick < i) {
			...
			int i = totalLength / 5;
			while (i != 0 && (totalLength % i != 0 || (totalLength / i) < maxStick)) {
				i--;
			}
			
			for (; i > 0; i--) { 
				if (totalLength % i == 0) {
					if (solution.canBeDivided(count, sticks, totalLength / i, i)) { 
